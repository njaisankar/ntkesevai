...
import {
  ...
  getTownPanchayatVillageStreetDetails,
  getServiceDetails,
  getPanchayatDetails,
...
const ServiceRequestForm = ({
  ...
  const [selectedTownPanchayatVillageId, setSelectedTownPanchayatVillageId] =
    useState(""); // For the nested dropdown
  ...
  const [filteredTownPanchayatVillages, setFilteredTownPanchayatVillages] =
    useState([]);
  ...
  useEffect(() => {
    const fetchServiceDetailsData = async () => {
      ...
      try {
        ...
        console.log(
          "fetchServiceDetailsData method called ==> Service Id ",
          serviceId,
        );
  ...
  useEffect(() => {
    const fetchTownPanchayatsData = async () => {
      if (!token || !selectedDistrictId || selectedDistrictId === "none") {
        // Adjust dependency if town panchayats depend on block
        setTownPanchayats([]);
        return;
      }
      try {
        ...
        const response = await getTownPanchayatDetails(
          token,
          selectedDistrictId,
        ); // Placeholder, adjust as per your actual API
        ...
        console.log(
          "town panchayat details fetch method called...",
          response.data,
        );
      } catch (error) {
  ...
  useEffect(() => {
    ...
    if (selectedBlockId && townPanchayats.length > 0) {
      console.log("new filter user effets for town panchayat", selectedBlockId);
      setFilteredTownPanchayats(
        townPanchayats.filter(
          (p) => String(p.blockDetails) === String(selectedBlockId),
        ),
      );
    } else {
      setFilteredTownPanchayats([]);
  ...
  useEffect(() => {
    if (selectedBlockId && panchayats.length > 0) {
      ...
      setFilteredPanchayats(
        panchayats.filter(
          (rv) => String(rv.blockDetails) === String(selectedBlockId),
        ),
      );
    } else {
      setFilteredPanchayats([]);
  ...
  useEffect(() => {
    if (
      (selectedTownPanchayatId || selectedPanchayatId) &&
      townPanchayatVillages.length > 0
    ) {
      console.log(
        "Filter Town or Panchayat Villages based on Town Panchayat ID =>",
        selectedTownPanchayatId,
        " Panchayat Id =>",
        selectedPanchayatId,
        townPanchayatVillages,
      );
      setFilteredTownPanchayatVillages(
        townPanchayatVillages.filter(
          (rv) =>
            String(rv.town_panchayat_id) === String(selectedTownPanchayatId) ||
            String(rv.village_panchayat_id) === String(selectedPanchayatId),
        ),
      );
    } else {
      setFilteredTownPanchayatVillages([]);
  ...
  useEffect(() => {
    const fetchRecordForEdit = async () => {
      console.log(
        "fetchRecordForEdit method called with isEditMode: " +
          isEditMode +
          ", recordId: " +
          recordId +
          ", token: " +
      ...
      if (isEditMode && recordId && token) {
        ...
        try {
          ...
          var response = await getServiceRequestDetailsById(
            recordId,
            url,
            token,
          );
          ...
          const isUserAllowedToApprove =
            userData.isApprover || userData?.isSuperUser;
          ...
          console.log(
            "record.district.district_id ? String(record.district_id)",
          ...
          setFormData({
            ...
            block_id: record.block.block_id
              ? String(record.block.block_id)
              : "",
            town_panchayat_id: record.town_panchayat?.town_panchayat_id
              ? String(record.town_panchayat?.town_panchayat_id)
              : "",
            panchayat_id: record.panchayat?.panchayat_id
              ? String(record.panchayat?.panchayat_id)
              : "",
            village_street_id: record.village_street?.id
              ? String(record.village_street?.id)
              : "",
          ...
          setSelectedServiceId(
            record.service_id > 0 ? record.service_id : selectedServiceId,
          );
          setSelectedServiceDetailsId(
            record.service_details.service_details_id > 0
              ? record.service_details.service_details_id
              : selectedServiceDetailsId,
          );
          setSelectedDistrictId(
            record.district_id ? String(record.district_id) : "1",
          );
          ...
          if (record.town_panchayat?.town_panchayat_id) {
            ...
            setSelectedTownPanchayatId(
              record.town_panchayat?.town_panchayat_id,
            );
            ...
            !record.town_panchayat?.town_panchayat_id &&
            !record.panchayat?.panchayat_id &&
            !record.revenue_village_id
          ) {
    ...
    userData.isApprover,
    userData.id,
  ]); // Added townPanchayatVillages as dependency for street_name prefill
  ...
  const handleChange = (e) => {
    ...
    if (type === "file") {
      ...
      if (name === "block_id") {
        ...
        if (value) {
          setFilteredPanchayats(
            panchayats.filter((p) => String(p.blockDetails) === String(value)),
          );
          setFilteredTownPanchayats(
            townPanchayats.filter(
              (p) => String(p.blockDetails) === String(value),
            ),
          );
        } else {
          setFilteredPanchayats([]);
      ...
      if (name === "town_panchayat_id") {
        ...
        if (value) {
          setFilteredTownPanchayatVillages(
            townPanchayatVillages.filter(
              (rv) => String(rv.town_panchayat_id) === String(value),
            ),
          );
        } else {
          setFilteredTownPanchayatVillages([]);
      ...
      if (name === "panchayat_id") {
        ...
        if (value) {
          ...
          setFilteredTownPanchayatVillages(
            townPanchayatVillages.filter(
              (rv) => String(rv.village_panchayat_id) === String(value),
            ),
          );
        } else {
          setFilteredTownPanchayatVillages([]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (formData.document_attachment instanceof File) {
        ...
        submitData.append(
          "document_attachment",
          formData.document_attachment,
          formData.document_attachment.name,
        );
      }
      ...
      if (isEditMode && recordId) {
        ...
        response = await UpdateServiceRequestDetails(
          recordId,
          submitData,
          token,
        );
    ...
    <form onSubmit={handleSubmit} autoComplete="off">
      ...
      <div className="row g-3">
        ...
        <div className="col-12 col-md-4">
          <label htmlFor="service_details_id" className="form-label">
            வேலைகள் {userData?.isCreator} {userData?.isApprover}
          </label>
          <select
          ...
          >
            ...
            {(serviceDetails || []).map((service) => (
              <option
                key={service.service_details_id}
        ...
        <div className="col-12 col-md-4">
          <label htmlFor="shop_num" className="form-label">
            கடை எண்
          </label>
          <input
            type="text"
        ...
        <div className="col-12 col-md-4">
          <label htmlFor="block_town_type" className="form-label">
            அரசுப் பிரிவு
          </label>
          <select
            id="block_town_type"
        ...
        {selectedBlockTownId === "1" && ( // "ஒன்றியம்"
          <>
            ...
            <div className="col-12 col-md-4">
              <label htmlFor="panchayat_id" className="form-label">
                ஊராட்சி
              </label>
              <select
                id="panchayat_id"
        ...
        {selectedBlockTownId === "2" && ( // "பேரூராட்சி"
          <>
            ...
            <div className="col-12 col-md-4">
              <label htmlFor="town_panchayat_id" className="form-label">
                பேரூராட்சி
              </label>
              <select
              ...
              >
                ...
                {(filteredTownPanchayats || []).map((tp) => (
                  <option
                    key={tp.town_panchayat_id}
                    value={tp.town_panchayat_id}
                  >
                    {tp.name}
                  </option>
                ))}
        ...
        {(selectedBlockTownId === "1" || selectedBlockTownId === "2") && (
          <div className="col-12 col-md-4">
            <label htmlFor="village_street_id" className="form-label">
              தெரு பெயர் / கிராமம்
            </label>
            <select
        ...
        <div className="col-12 col-md-4">
          <label htmlFor="registered_mobile" className="form-label">
            பதிவு செய்யப்பட்ட கைபேசி
          </label>
          <input
        ...
        <div className="col-12 col-md-4">
          <label htmlFor="document_attachment" className="form-label">
            சான்றிதழை பதிவேற்று
          </label>
          <input
            type="file"
